...
void step(body_t* target, attractor_t* attractors, int nattractors, double t){
    ...
    accel.x = fmax(-0.01f, fmin(0.01f, accel.x));
    accel.y = fmax(-0.01f, fmin(0.01f, accel.y));
    ...
    // new velocity is amount move in time step over time
...
int closest(vector2 pos, attractor_t* bodies, int nbodies){
    ...
    for(int i = 0; i < nbodies; i++){
        float dist = distance(bodies[i].pos, pos);
        if(dist < cur_dist){
            cur = i;
            cur_dist = dist;
        }
